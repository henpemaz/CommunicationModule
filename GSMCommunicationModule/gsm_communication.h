...
///<summary>Configure Serial and IO pins to operate the communication module. If the module is ON (shouldn't be on boot), turn it OFF</summary>
///<returns>Always returns COMM_OK, no check is performed</returns>
enum comm_status_code comm_setup(void);

///<summary>Turn on the module, connect to GPRS, start an HTTP session and prepare to send POST data</summary>
///<returns>Returns COMM_OK if the module is connected and a POST DATA session was open. 
///			Returns COMM_ERR_RETRY if the boot or some of the issued commands failed.
///			Returns COMM_ERR_RETRY_LATER if the timeout of the GPRS subscription was reached</returns>
